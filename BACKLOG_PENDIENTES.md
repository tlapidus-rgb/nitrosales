...
| **N-09** | **`Order`, `Product` y `Customer` tienen clave única `(organizationId, externalId)` SIN la plataforma**, y `Product`/`Customer` ni siquiera tienen columna `source` | Salió de E-30 (2026-09-13). **Hoy no colisiona** y por eso no se arregló: cada plataforma usa una forma distinta de id (`ml-123` vs UUID de VTEX para clientes; `MLA123` vs numérico para productos), así que los espacios no se pisan. Es deuda estructural, no un bug activo — pero el día que entre una plataforma con ids numéricos planos, dos productos distintos pasan a ser el mismo y el upsert pisa uno con otro, **en silencio**. Arreglarlo es agregar `source` a las dos tablas y migrar la clave: SQL en Neon, o sea que lo corre Axel. Antes hay que decidir qué pasa con las filas existentes, que no tienen plataforma asignada |
| **N-08** | **Aurum no usa prompt caching en ninguna llamada** — no hay un solo `cache_control` en todo el repo, y cada request manda el system prompt + las 12 definiciones de tools de nuevo | Salió de E-21 (2026-09-12), al costear Aurum por primera vez. Es la palanca de costo más grande y más barata que hay: el prefijo repetido se cobra entero en cada query, y cachearlo lo baja ~90 % en la parte cacheada. **Antes de tocarlo hay que medir**, que ahora se puede: `/api/admin/consumo-por-cliente` da el USD por cliente. Segundo paso: los logs no guardan tokens de cache, así que si se activa el caching hay que sumar esas columnas a `aurum_usage_logs` o el costeo queda **alto** y nadie se entera. Ojo con el orden: activar el caching sin tocar los logs hace que el ahorro sea invisible justo en el reporte que lo tendría que mostrar |
| **N-07** | **¿El paquete "sólo NitroPixel" se puede dar de alta solo?** Hoy **no**: `submit-wizard` exige al menos una plataforma y el pixel se filtra antes de mandar, así que un alta de sólo pixel se rechaza con 400 | Salió de E-29 (2026-09-12). El wizard ya no miente —avisa que el pixel solo no alcanza, en vez de mostrar 100 % y tirar 400—, pero eso **describe** el comportamiento actual, no lo valida. Y "sólo el pixel" no es hipotético: es lo que se le vendió a TeVeCompras, hoy resuelto editando un rol a mano en la base (ver E-22). Si ese paquete se va a vender de verdad, el alta tiene que soportarlo sin intervención manual. **Es decisión de Tomy, no un bug**: puede que el pixel solo efectivamente no alcance para que el producto sirva, y en ese caso el mensaje actual es correcto y esto se cierra sin código |
| **N-06** | **~50 endpoints comparan `NEXTAUTH_SECRET` con `!==` directo y NO toleran la ventana de rotación** (`/api/sync/prices`, `/api/sync/catalog`, `/api/sync/vtex`, los `migrate-*`, el webhook de inventory, `/api/health`) | El 2026-09-12 se prepararon `ADMIN_API_KEY` y el webhook de órdenes para rotar por etapas. **Estos quedaron afuera**: durante una ventana de rotación aceptarían sólo la clave nueva. Los crons principales sobreviven porque aceptan además `ADMIN_API_KEY`, que ya es rotable; los de arriba, no. No es urgente —sin rotación no pasa nada— pero **hay que hacerlo antes de rotar de verdad**, o esos endpoints se caen justo durante la ventana que existe para que nada se caiga. El arreglo es mecánico: importar `esClaveDeWebhookValida` de `@/lib/webhook-key`. Lo que NO es mecánico es decidir si además merecen su propio secreto, separado del que firma las sesiones de NextAuth |
