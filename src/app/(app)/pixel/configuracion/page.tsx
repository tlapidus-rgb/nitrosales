  ...
  return (
    <div className="min-h-screen bg-canvas">
      <div className="max-w-4xl mx-auto px-6 py-8">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-ink">Configuración de NitroPixel</h1>
        ...
        <div className="mb-6">
          <h2 className="text-xl font-bold text-ink">Constructor de URL & Tagueo</h2>
        ...
        {/* Constructor de UTMs */}
        <div className="bg-elevated rounded-2xl border border-hairline p-6 mb-6 shadow-sm">
          <h2 className="text-lg font-semibold text-ink mb-1">Constructor de URL</h2>
          ...
          <div className="space-y-4">
            {/* URL base */}
            <div>
              <label className="block text-xs font-medium text-ink-60 mb-1.5">URL de la página</label>
              <input
                type="url"
                value={baseUrl}
            ...
            {/* Preset */}
            <div>
              <label className="block text-xs font-medium text-ink-60 mb-1.5">¿Dónde la vas a publicar?</label>
              <select
                value={presetIdx}
                onChange={(e) => setPresetIdx(Number(e.target.value))}
                className="w-full px-3 py-2 border border-hairline rounded-lg text-sm focus:ring-2 focus:ring-accent/30 focus:border-accent outline-none bg-elevated"
              >
                {PRESETS.map((p, i) => (
                  <option key={i} value={i}>{p.label}</option>
                ))}
              </select>
              <p className="text-[11px] text-ink-40 mt-1">{preset.description}</p>
            </div>

            {/* Campaign + Content */}
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-ink-60 mb-1.5">Nombre de la campaña</label>
                <input
                  type="text"
                  value={campaign}
                ...
                />
                {campaign && (
                  <p className="text-[11px] text-ink-40 mt-1">→ <code>{slugify(campaign)}</code></p>
                )}
              </div>
              <div>
                <label className="block text-xs font-medium text-ink-60 mb-1.5">
                  Variante <span className="text-ink-40">(opcional)</span>
                </label>
                <input
                  type="text"
                  value={content}
                ...
                />
                {content && (
                  <p className="text-[11px] text-ink-40 mt-1">→ <code>{slugify(content)}</code></p>
                )}
              </div>
            </div>
            ...
            {/* URL final */}
            <div className="pt-2">
              <label className="block text-xs font-medium text-ink-60 mb-1.5">URL final</label>
              {finalUrl ? (
                <div className="bg-surface rounded-lg p-3 border border-hairline group relative">
                  <code className="text-xs text-ink-60 break-all block pr-20">{finalUrl}</code>
                  <button
                    onClick={handleCopy}
                    className="absolute top-2 right-2 px-3 py-1 text-xs font-medium bg-accent hover:bg-accent text-white rounded transition-colors"
                  >
                    {copied ? "Copiado ✓" : "Copiar"}
                  </button>
                </div>
              ) : (
                <div className="bg-surface rounded-lg p-3 border border-dashed border-hairline">
                  <p className="text-xs text-ink-40 italic">Empezá completando los campos de arriba</p>
                </div>
              )}
            </div>
          </div>
        ...
        {/* Guía de tagueo */}
        <div className="bg-elevated rounded-2xl border border-hairline p-6 mb-6 shadow-sm">
          <h2 className="text-lg font-semibold text-ink mb-1">Guía rápida de tagueo</h2>
          <p className="text-xs text-ink-40 mb-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-hairline">
                {PRESETS.map((p, i) => (
                  <tr key={i} className="text-ink-60">
                    <td className="px-3 py-2 font-medium">{p.label}</td>
                    <td className="px-3 py-2"><code className="bg-surface px-1.5 py-0.5 rounded">{p.source}</code></td>
                    <td className="px-3 py-2"><code className="bg-surface px-1.5 py-0.5 rounded">{p.medium}</code></td>
                    <td className="px-3 py-2 text-ink-40">{p.description}</td>
                  </tr>
                ))}
              </tbody>
            </table>
