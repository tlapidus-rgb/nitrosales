      ...
      try {
        ...
        const parsedProducts = data.products.map(p => ({
          ...
          trendData: {
            ...
            // Map weeklyData (API v3) â weeklyTrend (v10 page)
        ...
        // Map stockHealthSummary (API v3) â stockSummary (v10 page)
        ...
        <div className="bg-gradient-to-r from-amber-50 to-orange-50 p-6 rounded-xl border border-amber-200">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            <div className="bg-white/80 rounded-lg p-3">
              ...
              <p className="text-xl font-bold text-amber-800">{data.bagsAnalytics.totalBagsSold.toLocaleString("es-AR")}</p>
            </div>
            <div className="bg-white/80 rounded-lg p-3">
              <p className="text-xs text-gray-500 mb-1">Revenue</p>
              <p className="text-xl font-bold text-amber-800">{"$" + data.bagsAnalytics.bagsRevenue.toLocaleString("es-AR")}</p>
            </div>
            <div className="bg-white/80 rounded-lg p-3">
              <p className="text-xs text-gray-500 mb-1">Stock Actual</p>
              <p className="text-xl font-bold text-amber-800">{data.bagsAnalytics.currentStock.total.toLocaleString("es-AR")}</p>
              <p className="text-[10px] text-gray-400 mt-0.5">Grande: {data.bagsAnalytics.currentStock.grande.toLocaleString("es-AR")} | Chica: {data.bagsAnalytics.currentStock.chica.toLocaleString("es-AR")}</p>
            </div>
            <div className="bg-white/80 rounded-lg p-3">
              <p className="text-xs text-gray-500 mb-1">Adopción</p>
              <p className="text-xl font-bold text-amber-800">{data.bagsAnalytics.bagAdoptionPct}%</p>
              <p className="text-[10px] text-gray-400 mt-0.5">{data.bagsAnalytics.ordersWithBags} de {data.bagsAnalytics.totalOrders} órdenes</p>
              <div className="w-full bg-amber-200 rounded-full h-1.5 mt-1.5">
                <div className="bg-amber-600 h-1.5 rounded-full transition-all" style={{ width: Math.min(data.bagsAnalytics.bagAdoptionPct, 100) + "%" }} />
              </div>
            </div>
          </div>
          {data.bagsAnalytics.breakdown.length > 0 && (
            <div className="flex gap-2 flex-wrap">
              {data.bagsAnalytics.breakdown.map((bag: any, i: number) => (
