...
function buildProductsPrompt(data: any) {
  ...
  const top5 = prods.slice(0, 5).map((p: any, i: number) =>
...
function buildCampaignsPrompt(data: any) {
  ...
  const totalRev = camps.reduce((s: number, c: any) => s + c.conversionValue, 0);
  ...
  const gRev = google.reduce((s: number, c: any) => s + c.conversionValue, 0);
  const mRev = meta.reduce((s: number, c: any) => s + c.conversionValue, 0);
  const campList = camps.slice(0, 8).map((c: any, i: number) =>
  ...
  try {
    ...
    if (!dataPrompt) {
      return NextResponse.json({
        insights: [{ type: "TREND", title: "Sin datos suficientes", description: "No hay datos disponibles para generar insights en esta seccion.", action: "Verifica que los conectores esten activos en Configuracion." }]
      });
    }
...
{"insights":[{"type":"ALERT|OPPORTUNITY|TREND","title":"max 10 palabras","description":"max 40 palabras con datos concretos del contexto","action":"1 accion especifica y concreta"}]}`,
    ...
    return NextResponse.json({
      insights: [{ type: "ALERT", title: "Error generando insights", description: "Hubo un problema al analizar los datos. Intenta recargar la pagina.", action: "Recargar pagina" }]
    });
  }
}
