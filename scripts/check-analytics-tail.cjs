...
const expectedSql = sql(before)
  ...
  .map(query => query
    ...
    .replace(
      ...
      'LEFT JOIN LATERAL ( SELECT candidate."orderId" FROM pixel_attributions candidate WHERE candidate."orderId" = o.id AND candidate.model = CAST(${selectedModel} AS "AttributionModel") LIMIT 1 ) pa ON TRUE'
    ))
